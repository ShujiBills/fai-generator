...
const WO_FOLDER = 'O:\\Production (US)\\WORK ORDERS';
...
function findWoPdfs(woNumber) {
  ...
  // Search both Q: drive FAI folders and O: drive WORK ORDERS folder
  const searchPaths = [];

  if (fs.existsSync(WIP_FAI_ROOT)) {
    const faiDirs = fs.readdirSync(WIP_FAI_ROOT, { withFileTypes: true })
      .filter(d => d.isDirectory())
      .map(d => path.join(WIP_FAI_ROOT, d.name));
    searchPaths.push(...faiDirs);
  }

  if (fs.existsSync(WO_FOLDER)) {
    searchPaths.push(WO_FOLDER);
  }

  if (searchPaths.length === 0) return { exact, generic };

  const faiDirs = searchPaths;
  ...
  if (!pdfPath) {
    return {
      ...
      message: `WO picking list not found for WO# ${wo}. ` +
        `Save the WO PDF as "WO ${wo}.pdf" in O:\\Production (US)\\WORK ORDERS\\ and try again.`,
    };
  }
